#!/bin/bash
# =============================================================================
# SO101 叠放采集：蓝块进黄盘，白块叠在蓝块上（黄盘位置固定）
# Usage:
#   bash get-data-blue_white_stack.sh              # 开始采集（默认 120 条）
#   bash get-data-blue_white_stack.sh cameras      # 列出相机
#   bash get-data-blue_white_stack.sh preview      # Rerun GUI 看 front/wrist
#   bash get-data-blue_white_stack.sh viz          # 可视化已采集数据
#
# 覆盖参数示例:
#   NUM_EPISODES=10 bash get-data-blue_white_stack.sh
#   RESUME=1 bash get-data-blue_white_stack.sh
#
# 成功标准:
#   黄盘内：蓝块在下、白块稳定叠在蓝块上，再结束 episode
# =============================================================================
set -euo pipefail
...
# ---------- 数据集 ----------
DATA_REPO="${DATA_REPO:-my_pick_place/blue_white_stack_yellow_tray}"
DATA_ROOT="${DATA_ROOT:-/home/rxn/datasets/blue_white_stack_yellow_tray}"
SINGLE_TASK="${SINGLE_TASK:-put the blue block into the yellow tray, then place the white block on top of the blue block}"
...
echo "=============================="
echo "SO101 blue→tray + white→blue (${MODE})"
echo "=============================="
echo "Task:       ${SINGLE_TASK}"
echo "Output:     ${DATA_ROOT}"
echo "Episodes:   ${NUM_EPISODES}  (${EPISODE_TIME_S}s/ep, reset ${RESET_TIME_S}s)"
echo "Scene tip:  yellow tray fixed; shuffle blue/white start poses"
echo "Success:    blue in tray, white stably on blue"
...
if [ "${MODE}" != "record" ]; then
    echo "Usage: bash get-data-blue_white_stack.sh [record|cameras|preview|viz]"
    exit 1
fi
...
if failed:
    print("ERROR: camera check failed. Run: bash get-data-blue_white_stack.sh cameras", file=sys.stderr)
    sys.exit(1)
PY
...
echo "Viz:     bash get-data-blue_white_stack.sh viz"
echo "Train tip: DATA_ROOT=${DATA_ROOT} bash train_smolvla_blue_block.sh  # or copy that script"
echo "=============================="
