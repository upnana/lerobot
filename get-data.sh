...
# SO101 遥操作数据采集
# Usage:
#   bash get-data.sh              # 开始采集（默认：蓝块 → yellow tray，90 条）
#   bash get-data.sh cameras      # 列出相机
#   bash get-data.sh preview      # Rerun GUI 看 front/wrist（颜色正确）
#   bash get-data.sh viz          # 可视化已采集数据
#
# 覆盖参数示例:
#   NUM_EPISODES=10 bash get-data.sh
#   DATA_ROOT=/home/rxn/datasets/my_run bash get-data.sh
#   RESUME=1 bash get-data.sh           # 从已有数据继续采到 NUM_EPISODES 条
# =============================================================================
set -euo pipefail
...
SINGLE_TASK="${SINGLE_TASK:-pick up the blue block and put it into the yellow tray}"
NUM_EPISODES="${NUM_EPISODES:-90}"
EPISODE_TIME_S="${EPISODE_TIME_S:-40}"
RESET_TIME_S="${RESET_TIME_S:-5}"
PUSH_TO_HUB="${PUSH_TO_HUB:-false}"
# Rerun 可视化会拖慢控制循环；与双臂采集一致，默认关闭（需要时 DISPLAY_DATA=true）
...
echo "=============================="
echo "SO101 Data Collection (${MODE})"
echo "=============================="
...
if [ "${MODE}" != "record" ]; then
    echo "Usage: bash get-data.sh [record|cameras|preview|viz]"
    exit 1
fi
...
if failed:
    print("ERROR: camera check failed. Run: bash get-data.sh cameras", file=sys.stderr)
    sys.exit(1)
PY
