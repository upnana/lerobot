#!/bin/bash
# =============================================================================
# SmolVLA 训练（yellow_white_base2 单任务）
# Usage:
#   conda activate lerobot
#   bash train_smolvla_base2.sh              # 单卡 GPU1 前台
#   BACKGROUND=1 bash train_smolvla_base2.sh # 后台
#   RESUME=1 bash train_smolvla_base2.sh     # 从 last checkpoint 续训
#   FRESH=1 bash train_smolvla_base2.sh      # 删 output 重训
#   tail -f logs/smolvla_yellow_white_base2.log
# =============================================================================
set -euo pipefail

PROJECT_ROOT="/home/rxn/lerobot"
DATA_ROOT="/home/rxn/datasets/yellow_white_base2"
DATA_REPO="my_pick_place/yellow_white_base2"
...
JOB_NAME="${JOB_NAME:-smolvla_yellow_white_base2}"
...
STEPS="${STEPS:-100000}"        # ~20 epoch @ bs=4
...
if [ "${BACKGROUND:-0}" = "1" ] && [ -z "${TRAIN_SMOLVLA_BASE2_BG:-}" ]; then
    ...
    echo "  log: ${LOG_FILE}"
    nohup env TRAIN_SMOLVLA_BASE2_BG=1 \
...
[ "${NUM_GPUS}" -gt 1 ] && ACCEL_ARGS+=(--multi_gpu --main_process_port="${MAIN_PROCESS_PORT:-29503}")
...
echo "Infer: CHECKPOINT=${OUTPUT_DIR}/checkpoints/last/pretrained_model bash infer_smolvla_base2.sh robot"
echo "=============================="
