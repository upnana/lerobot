#!/bin/bash
# =============================================================================
# SmolVLA 推理（yellow_white_base2 单任务）
# Usage:
#   conda activate lerobot
#   bash infer_smolvla_base2.sh robot      # 真机推理
#   bash infer_smolvla_base2.sh offline    # 数据集离线推理
#   bash infer_smolvla_base2.sh cameras    # 列出相机
#   bash infer_smolvla_base2.sh viz        # 可视化训练集
#
# 指定 checkpoint:
#   CHECKPOINT=.../checkpoints/050000/pretrained_model bash infer_smolvla_base2.sh robot
#   EPISODE_TIME_S=90 NUM_EPISODES=3 CUDA_VISIBLE_DEVICES=1 bash infer_smolvla_base2.sh robot
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/outputs/train/smolvla_yellow_white_base2}"
...
EVAL_ROOT="${EVAL_ROOT:-${PROJECT_ROOT}/outputs/eval/smolvla_yellow_white_base2}"
...
echo "=============================="
echo "SmolVLA base2 Inference (${MODE})"
...
if [ "${MODE}" = "cameras" ]; then
    ...
    echo ""
    echo "Defaults: FRONT_CAM=${FRONT_CAM}  (OpenCV index 0)"
    echo "          WRIST_CAM=${WRIST_CAM}  (OpenCV index 4)"
    echo ""
...
if [ "${MODE}" = "viz" ]; then
    if [ -d "${EVAL_ROOT}" ] && [ -n "$(ls -A "${EVAL_ROOT}" 2>/dev/null)" ]; then
        ...
        lerobot-dataset-viz \
          --repo-id="upna/eval_smolvla_yellow_white_base2" \
          --root="${LATEST_EVAL}" \
          --episode-index=0 \
          --mode=local
    else
...
print("Running offline inference on base2 dataset...")
...
if failed:
    print("ERROR: camera check failed. Run: bash infer_smolvla_base2.sh cameras", file=sys.stderr)
    sys.exit(1)
PY

    echo ""
    echo ">>> Starting SmolVLA base2 inference in 3s (Ctrl+C to abort)..."
    sleep 3
    ...
    lerobot-record \
      ...
      --dataset.repo_id="upna/eval_smolvla_yellow_white_base2" \
...
for raw in sys.stdin:
    ...
    if 'Recording episode' in line:
        ...
        shown = False
        print(); print('=' * 62)
        print('  SmolVLA base2 推理中 (${EPISODE_TIME_S}s)')
...
"
    ...
    echo "Usage: bash infer_smolvla_base2.sh [robot|offline|cameras|viz]"
    exit 1
fi
