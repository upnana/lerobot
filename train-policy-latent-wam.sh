#!/bin/bash
# Latent WAM-lite finetune of SmolVLA (Evo-RL feat/latent-wam).
#
# Does NOT change stock SmolVLA defaults: WAM is opted in via CLI flags only.
# Original ACP script (train-policy-evo-rl-acp.sh) is unchanged.
#
# Usage:
#   bash /home/rxn/lerobot/train-policy-latent-wam.sh
#   NUM_GPUS=2 BATCH_SIZE=4 STEPS=80000 bash ...
#   FRESH=1 BACKGROUND=1 bash ...
set -euo pipefail
...
RUN_NAME="${RUN_NAME:-smolvla_latent_wam_snacks_bowls_v1}"
...
# Latent WAM knobs (only used by this script)
LATENT_HORIZON="${LATENT_HORIZON:-16}"
LATENT_DIM="${LATENT_DIM:-256}"
LATENT_LOSS_WEIGHT="${LATENT_LOSS_WEIGHT:-0.1}"
USE_LATENT_CONDITIONING="${USE_LATENT_CONDITIONING:-true}"

if [ "${BACKGROUND:-0}" = "1" ] && [ -z "${TRAIN_WAM_BG:-}" ]; then
  ...
  echo "Starting background latent-WAM policy train..."
  nohup env TRAIN_WAM_BG=1 \
    ...
    LATENT_HORIZON="${LATENT_HORIZON}" LATENT_DIM="${LATENT_DIM}" \
    LATENT_LOSS_WEIGHT="${LATENT_LOSS_WEIGHT}" \
    USE_LATENT_CONDITIONING="${USE_LATENT_CONDITIONING}" \
    bash "$0" > "${LOG_FILE}" 2>&1 &
  echo $! > "${PID_FILE}"
...
print(f"Latent WAM: horizon=${LATENT_HORIZON} dim=${LATENT_DIM} weight=${LATENT_LOSS_WEIGHT}")
PY
...
echo "=============================="
echo "Latent WAM policy train (${RUN_NAME})"
echo "=============================="
echo "Env:       ${CONDA_ENV}"
echo "Branch:    feat/latent-wam (Evo-RL)"
...
[ "${NUM_GPUS}" -gt 1 ] && ACCEL_ARGS+=(--multi_gpu --main_process_port="${MAIN_PROCESS_PORT:-29508}")
...
POLICY_ARGS=(
  ...
  --policy.use_latent_wam=true
  --policy.latent_horizon="${LATENT_HORIZON}"
  --policy.latent_dim="${LATENT_DIM}"
  --policy.latent_loss_weight="${LATENT_LOSS_WEIGHT}"
  --policy.use_latent_conditioning="${USE_LATENT_CONDITIONING}"
)
