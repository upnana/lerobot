#!/bin/bash
# =============================================================================
# SmolVLA 推理：bimanual stack bowls（黄→棕→白）· ~20 epoch（ckpt 120000）
#
# 默认用 checkpoints/120000（保存点 ≈19.4 epoch；日志里 step≈123K 才到 20.0，
# 最近可用保存点是 120000）。
#
# Usage:
#   conda activate lerobot
#   bash infer_smolvla_bimanual_stack_bowls_ep20.sh offline
#   bash infer_smolvla_bimanual_stack_bowls_ep20.sh robot
#   bash infer_smolvla_bimanual_stack_bowls_ep20.sh cameras
#   bash infer_smolvla_bimanual_stack_bowls_ep20.sh preview
#   bash infer_smolvla_bimanual_stack_bowls_ep20.sh viz
#
# 换 ckpt:
#   CHECKPOINT=.../checkpoints/130000/pretrained_model bash infer_smolvla_bimanual_stack_bowls_ep20.sh robot
# =============================================================================
set -euo pipefail
...
CHECKPOINT="${CHECKPOINT:-${OUTPUT_DIR}/checkpoints/130000/pretrained_model}"  # ~20 ep 最近保存点 (≈19.4)
...
EVAL_ROOT="${EVAL_ROOT:-${PROJECT_ROOT}/outputs/eval/smolvla_bimanual_stack_bowls_ep20}"
...
echo "=============================="
echo "SmolVLA Bimanual stack bowls ep≈20 Inference (${MODE})"
...
init_rerun(session_name="smolvla_stack_bowls_cam_preview")
...
if [ "${MODE}" = "viz" ]; then
    if [ -d "${EVAL_ROOT}" ] && [ -n "$(ls -A "${EVAL_ROOT}" 2>/dev/null)" ]; then
        ...
        lerobot-dataset-viz \
          --repo-id="upna/eval_smolvla_bimanual_stack_bowls_ep20" \
          --root="${LATEST_EVAL}" \
          --episode-index=0 \
...
print("Running offline inference (ckpt ~ep20 / 120000)...")
...
if failed:
    print("ERROR: camera check failed. Run: bash infer_smolvla_bimanual_stack_bowls_ep20.sh cameras", file=sys.stderr)
    sys.exit(1)
PY
    ...
    lerobot-record \
      ...
      --dataset.repo_id="upna/eval_smolvla_bimanual_stack_bowls_ep20" \
...
"
    ...
    echo "Usage: bash infer_smolvla_bimanual_stack_bowls_ep20.sh [robot|offline|preview|cameras|viz]"
    exit 1
fi
