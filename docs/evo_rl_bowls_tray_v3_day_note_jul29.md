...
> **完整面试问答清单：** `docs/evo_rl_interview_qa.md`
> **ACP tag A/B（2026-08-01）：** `docs/evo_rl_acp_tag_ab_aug01.md`

---

## 电梯演讲（约 45 秒）

今天我在双臂 SO101 上做了 **闭环评测 + 价值函数诊断 + 数据迭代**。真机表现可按**子任务拆开讲**：**叠碗顺序基本能完成**；卡点在后半 **口红 + 纸巾入黄 tray**，做不完整。对 value overlay 定量后发现，口红未抓住时 \(V(s)\) 仍抬升（假进度）。据此做了失败聚焦 HITL（+56）→ merge v3（287 eps）→ 重训 value/policy。

**一句话结论：** 长程任务要报 **分阶段能力**，不能只说「成/败」；value 好看 ≠ 难子任务完成。
...
| 任务 | 零食入黄 tray + 叠碗 |
| 数据 | 清洗后约 **69**，后扩到约 **201** |
| 结果 | 流水线通，但真机基本 **hold / 学不会** |
| 主因（复盘） | 相机是 **partial 近景/局部视角**，**没有全视野顶视**，长程桌面状态观测不足 |

> 面试别把「零食+叠碗」说成叠碗+口红的第一轮；那是**换任务之前的失败复盘**，用来说明为什么后来采集加上全视野相机。

### 正题三次：叠碗 + 口红 / 纸巾

> 「HITL 接入」= 策略 rollout + 人干预后 **merge 进训练集** 的条数。

| 轮次 | 内容 | 基座 | 本轮 HITL | 训练总 eps | 备注 |
|---|---|---|---|---|---|
| **①** | 新任务首采 + 首训 | 遥操作 **150**（采集含全视野；训练用 **3 相机**） | **0** | **150** | 相对零食任务：有全视野采集，质检后约 77% 成功标注 |
| **②** | 首轮 HITL 纠错 merge → v2 | **150** | **+81**（2026-07-28） | **231** | 成功 178 / 失败 53 |
| **③** | 口红失败聚焦再 HITL → v3 | v2 **231** | **+56**（2026-07-29） | **287** | 成功 217 / 失败 70 |

**累计：** 遥操作 150 + HITL **81+56=137** → **287** 条。

**真机能力（①–③）：** 叠碗顺序基本能完成；口红 + 纸巾仍做不完整。

---

## 真机能力分层（面试必报）

整任务尚未跑出严格闭环 SR，但阶段表现已经清楚：

| 子任务 | 真机表现 | 说明 |
|---|---|---|
| 叠碗（黄→棕→白 → 白盘） | **基本能完成** | 顺序与堆叠流程大体学会 |
| 口红抓放进黄 tray | **做不完整** | 常够到/试抓但抓不起或放不稳 |
| 蓝纸巾入黄 tray | **做不完整** | 与口红同属后半段精细操作瓶颈 |

**面试说法：**

> 我不会报一个笼统的任务成功率。当前模型在长程双臂任务上呈现 **前半程（叠碗）可用、后半程（口红/纸巾）未打通**。诊断重点因此放在后半抓取与 value 假进度，而不是再堆无关的碗数据。

这样即使没有整任务 SR，也显得评估是 **按子技能拆解的**，比「就是不太行」专业。
...
| 步骤 | 内容 | 结果 |
|---|---|---|
| 1. 真机 eval | v2 policy，中期 **90k**，`PRESET=n10_acp_none` | **叠碗基本 OK**；**口红/纸巾做不完** |
...
## 面试 FAQ：没有 SR / latency 怎么说？

**Q：成功率多少？Latency 多少？**  
**A：** 还没做 N≥20 的严格闭环 SR 表和分层 latency。现在有的是 **分阶段真机结论**：叠碗顺序基本完成，口红/纸巾未打通；再加 value/HITL 诊断。数据里的 success% 是 **demo 标注**，不是 policy SR。下一步会按固定场景补 SR（分整任务 + 分阶段）和 latency（infer / 控制周期 / 完成时间）。
...
| 真机能力 | **叠碗基本 OK**；瓶颈在 **口红 + 纸巾** |
| 真机扫点 | 优先 v3 **80–120k**；对照 **v2 @ 120k** |
| Value | overlay 重点看后半：未抓住口红时 \(V\) 是否仍虚高 |
| 数据 | 下一轮优先补 **口红/纸巾成功 demo**；过滤极慢 HITL；少堆已会的叠碗 |
