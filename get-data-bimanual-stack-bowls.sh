#!/bin/bash
# =============================================================================
# SO101 双臂叠碗采集：黄碗 → 棕碗 → 白碗（自下而上）
# Usage:
#   bash get-data-bimanual-stack-bowls.sh              # 开始采集（默认 200 条）
#   bash get-data-bimanual-stack-bowls.sh cameras
#   bash get-data-bimanual-stack-bowls.sh preview4
#   bash get-data-bimanual-stack-bowls.sh viz
#
# 覆盖参数示例:
#   NUM_EPISODES=10 bash get-data-bimanual-stack-bowls.sh
#   RESUME=1 NUM_EPISODES=250 bash get-data-bimanual-stack-bowls.sh  # 注意大写 RESUME
#   EPISODE_TIME_S=100 bash get-data-bimanual-stack-bowls.sh
#
# 条数建议（三碗叠放 + 起始位姿随机大）:
#   目标 200；真机仍脆再补到 250–300。失败条用 ← 重录，质量 > 盲目加量。
#
# 成功标准:
#   自下而上稳定叠好：黄碗在底、棕碗居中、白碗在顶
#
...
# 固定路径，方便 RESUME；若要带时间戳新开一份：
#   DATA_ROOT=/home/rxn/datasets/bimanual_stack_bowls_$(date +%Y%m%d_%H%M%S) bash get-data-bimanual-stack-bowls.sh
...
echo "=============================="
echo "SO101 Bimanual stack bowls yellow→brown→white (${MODE})"
echo "=============================="
...
if [ "${MODE}" != "record" ]; then
    echo "Usage: bash get-data-bimanual-stack-bowls.sh [record|cameras|preview|preview4|rerun|viz]"
    exit 1
fi
...
if failed:
    print("ERROR: camera check failed. Run: bash get-data-bimanual-stack-bowls.sh cameras", file=sys.stderr)
    sys.exit(1)
PY
...
echo "Viz:     bash get-data-bimanual-stack-bowls.sh viz"
echo "Resume:  RESUME=1 NUM_EPISODES=200 bash get-data-bimanual-stack-bowls.sh"
echo "=============================="
