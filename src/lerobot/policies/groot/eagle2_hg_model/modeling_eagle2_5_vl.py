...
class Eagle2_5_VLPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["Qwen2DecoderLayer", "Siglip2EncoderLayer", "SiglipEncoderLayer"]
...
class Eagle2_5_VLForConditionalGeneration(Eagle2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Eagle2_5_VLConfig, vision_model=None, language_model=None):
        ...
        self.use_pixel_shuffle = getattr(config, "use_pixel_shuffle", False)
        ...
        if language_model is not None:
            ...
            if config.text_config.architectures[0] == 'LlamaForCausalLM':
                ...
                # assert config.text_config._attn_implementation == 'eager', f"Qwen2 must use eager but got {config.text_config._attn_implementation}"
        ...
        if self.use_pixel_shuffle:
            ...
            self.mlp1 = nn.Sequential(
                ...
                nn.Linear(mlp_in_dim, llm_hidden_size),
                nn.GELU(),
                nn.Linear(llm_hidden_size, llm_hidden_size),
            )
        else:
            # GR00T N1.5 checkpoint stores a single Linear projection at mlp1.0
            self.mlp1 = nn.Sequential(
    ...
    def forward(
        ...
        if (not torch.distributed.is_available()) or (not torch.distributed.is_initialized()) or torch.distributed.get_rank() == 0:
            logger.debug(
                "dynamic ViT batch size: %s, images per sample: %s, dynamic token length: %s",
                vit_batch_size,
                vit_batch_size / B,
                N,
            )
        ...
        try:
            ...
            logger.warning(
                "Vision/text token mismatch: %s, input_embeds[selected].shape=%s, vit_embeds.shape=%s",
                e,
    ...
    def extract_feature(self, pixel_values):
        ...
        # GR00T N1.5 (use_pixel_shuffle=false): mlp runs on full 256 patches, then pool to 64 tokens
        # to match tokens_per_tile=64 in the Eagle processor.
        if not self.use_pixel_shuffle and vit_embeds.shape[1] > self.num_image_token:
            batch_size, num_patches, channels = vit_embeds.shape
            side = int(num_patches**0.5)
            pool_size = side // int(self.num_image_token**0.5)
            vit_embeds = (
                vit_embeds.view(batch_size, side, side, channels)
                .permute(0, 3, 1, 2)
                .contiguous()
            )
            vit_embeds = F.avg_pool2d(vit_embeds, kernel_size=pool_size, stride=pool_size)
            vit_embeds = vit_embeds.permute(0, 2, 3, 1).reshape(batch_size, -1, channels)
...
Eagle25VLForConditionalGeneration = Eagle2_5_VLForConditionalGeneration
