#!/bin/bash
# =============================================================================
# SmolVLA 推理：bimanual stack bowls（黄→棕→白）· ~25 epoch（ckpt 150000）
#
# 默认用 checkpoints/150000（保存点 ≈24.3 epoch；日志里 step≈154K 才到 25.0，
# 最近可用保存点是 150000）。
#
# Usage:
#   conda activate lerobot
#   bash infer_smolvla_bimanual_stack_bowls_ep25.sh offline
#   bash infer_smolvla_bimanual_stack_bowls_ep25.sh robot
#   bash infer_smolvla_bimanual_stack_bowls_ep25.sh cameras
#   bash infer_smolvla_bimanual_stack_bowls_ep25.sh preview
#   bash infer_smolvla_bimanual_stack_bowls_ep25.sh viz
#
# 换 ckpt:
#   CHECKPOINT=.../checkpoints/160000/pretrained_model bash infer_smolvla_bimanual_stack_bowls_ep25.sh robot
# =============================================================================
set -euo pipefail
...
CHECKPOINT="${CHECKPOINT:-${OUTPUT_DIR}/checkpoints/150000/pretrained_model}"  # ~25 ep 最近保存点 (≈24.3)
CONDA_ENV="${CONDA_ENV:-lerobot}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"  # 训练占 0,1 时请改空闲卡或先停训
...
EVAL_ROOT="${EVAL_ROOT:-${PROJECT_ROOT}/outputs/eval/smolvla_bimanual_stack_bowls_ep25}"
...
echo "=============================="
echo "SmolVLA Bimanual stack bowls ep≈25 Inference (${MODE})"
...
init_rerun(session_name="smolvla_stack_bowls_cam_preview_ep25")
...
if [ "${MODE}" = "viz" ]; then
    if [ -d "${EVAL_ROOT}" ] && [ -n "$(ls -A "${EVAL_ROOT}" 2>/dev/null)" ]; then
        ...
        lerobot-dataset-viz \
          --repo-id="upna/eval_smolvla_bimanual_stack_bowls_ep25" \
          --root="${LATEST_EVAL}" \
          --episode-index=0 \
...
print("Running offline inference (ckpt ~ep25 / 150000)...")
...
if failed:
    print("ERROR: camera check failed. Run: bash infer_smolvla_bimanual_stack_bowls_ep25.sh cameras", file=sys.stderr)
    sys.exit(1)
PY
    ...
    lerobot-record \
      ...
      --dataset.repo_id="upna/eval_smolvla_bimanual_stack_bowls_ep25" \
...
for raw in sys.stdin:
    ...
    if 'Recording episode' in line:
        ...
        shown = False
        print(); print('=' * 62)
        print('  SmolVLA stack bowls 推理中 (${EPISODE_TIME_S}s)')
...
"
    ...
    echo "Usage: bash infer_smolvla_bimanual_stack_bowls_ep25.sh [robot|offline|preview|cameras|viz]"
    exit 1
fi
